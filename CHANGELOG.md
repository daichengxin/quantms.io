...
- **Unidentified Feature identity fallback**: an unidentified Feature (null `peptidoform`) with no producer id now derives a `feature_id` from the declared composite instead of raising. A producer-supplied id remains namespaced by run and preserved as a `provided_feature_id` cv_param.
