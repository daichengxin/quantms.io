...
- **OpenMS `-out_qpx` PSM run-file correction (major)**: OpenMS `-out_qpx` stamps **every** PSM row with the *first* run's file (the per-PSM `map_index` is dropped; OpenMS#9872), so on any multi-run / multi-fraction / multi-replicate design ~half the PSMs carry the wrong `run_file_name` — corrupting per-run stats and PSM identity. The OpenMS converter now reads the authoritative per-PSM run from the companion **consensusXML** (`qpx/converters/openms/psm_refs.py`) and restores each PSM's `run_file_name`, matched on the exact `(plain_sequence, charge, scan)` key (chemistry — peptidoform/modifications/mz — stays authoritative from the parquet). It also collapses OpenMS's assigned+unassigned duplicate PSMs (OpenMS#9871), preferring the feature-attached row. Verified on real cluster output (phospho: 496 rows all mislabeled `rep1` → 254 `rep1` / 238 `rep2`, 240 rows corrected, 4 duplicates merged, zero duplicate identities). When no consensusXML is supplied the converter falls back to the multi-engine merge.
- **Conversions no longer hard-fail on producer identity collisions or unidentified Features**: (a) a duplicate primary key is now reported as a **warning**, not an error, on both write paths (per-table strict validation and the whole-file check at close), so real producer data that collides on the identity composite — e.g. OpenMS `-out_qpx` writing one PSM row per search engine (same spectrum+peptidoform, different engine score) — still converts; a null id remains a hard error. (b) An **unidentified Feature** (null `peptidoform`) with no producer id now **derives** a `feature_id` from the composite (null peptidoform + measured `charge`/`run_file_name`/`rt`) instead of raising — OpenMS `-out_qpx` output containing unidentified features now converts. A producer-supplied id for an unidentified Feature is still namespaced by run and preserved as a `provided_feature_id` cv_param.
